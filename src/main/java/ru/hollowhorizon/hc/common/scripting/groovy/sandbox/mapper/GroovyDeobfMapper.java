package ru.hollowhorizon.hc.common.scripting.sandbox.mapper;
...
public class GroovyDeobfMapper {
    ...
    public static void init() {
        HollowCore.LOGGER.info("Generating obfuscation map...");
        DEOBF_METHODS.clear();
        DEOBF_FIELDS.clear();
        ...
        try {
            ...
            InputStream stream = GroovyDeobfMapper.class.getResourceAsStream("/mappings.srg");
            ...
            HollowCore.LOGGER.info("Read {} field and {} method mapping names", DEOBF_FIELDS.size(), DEOBF_METHODS.size());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static String getDeobfuscationMethodName(ClassNode owner, String methodName, Object[] args) {
        ClassNode objClass = ClassHelper.OBJECT_TYPE;
        Map<String, MethodInfo> obfNames;
        String obfName = null;

        do {
            obfNames = OBF_METHOD_NAMES.get(owner.getName());
            if (obfNames != null) {
                MethodInfo methodInfo = obfNames.get(methodName);
                if (methodInfo != null) {
                    obfName = methodInfo.findMethod(args);
                }
            }
            owner = owner.getSuperClass();
        } while (obfName == null && owner != null && owner != objClass);
    ...
    private static class MethodInfo {
        ...
        private MethodInfo(String deobfName, String defObfName, String defArgs) {
            this.deobfuscatedName = deobfName;
            this.obfuscatedName = defObfName;
            this.defArgs = defArgs;
        }

        public void registerOverloadedMethod(String obfName, String args) {
            if (obfName.equals(obfuscatedName)) return;
            if (obfNames == null) {
                obfNames = new ArrayList<>();
                obfNames.add(Pair.of(makeClassArray(defArgs), obfuscatedName));
            }
        ...
        public String findMethod(Parameter[] args) {
            if (this.obfNames == null) {
                String[] params = makeClassArray(defArgs);

                if(params.length != args.length) return null;

                for (int i = 0; i < args.length; i++) {
                    String origParam = params[i];
            ...
            HollowCore.LOGGER.error("Multiple methods match the name {} and params {}", this.deobfuscatedName, Arrays.toString(args));
            return results.get(0);
        }

        public static boolean matches(String original, Parameter param) {
            return matches(original, param.getOriginType());
        }

        public static boolean matches(String original, Object param) {
            return matches(original, new Parameter(new ClassNode(param.getClass()), "param"));
        }

        public static boolean matches(String original, ClassNode possibleMatch) {
        ...
        private static String[] makeClassArray(String descriptor) {
            ...
            try {
                ...
                HollowCore.LOGGER.info("An exception occured while creating a class array of arguments for {}", descriptor);
                e.printStackTrace();
            }
        ...
        public String findMethod(Object[] args) {
            if (this.obfNames == null) {
                String[] params = makeClassArray(defArgs);
            ...
            List<String> results = this.obfNames.stream()
                    ...
                    .map(pair -> {
                        HollowCore.LOGGER.info("{}: {}", Arrays.toString(pair.getKey()), pair.getValue());
