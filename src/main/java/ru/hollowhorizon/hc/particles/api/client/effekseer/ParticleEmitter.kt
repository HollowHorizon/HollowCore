...
import ru.hollowhorizon.hc.particles.api.client.effekseer.ParticleEmitter.PreDrawCallback
import java.util.*
...
class ParticleEmitter(private val handle: Int, private val manager: EffekseerManager, val type: Type?) {
    ...
    fun addPreDrawCallback(callback: PreDrawCallback) {
        Optional.ofNullable(this.callback).ifPresentOrElse(
            { c: PreDrawCallback ->
                this.callback = this.callback!!.andThen(c)
            },
    ...
    fun runPreDrawCallbacks(partial: Float) {
        Optional.ofNullable(this.callback).ifPresent { c: PreDrawCallback -> c.accept(this, partial) }
    }
}