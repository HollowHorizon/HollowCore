...
import ru.hollowhorizon.hc.particles.client.registry.EffectRegistry
import ru.hollowhorizon.hc.particles.common.util.Basis
import java.util.*
...
open class ParticleEmitterInfo : Cloneable {
    ...
    fun scale(): Vec3 {
        return if (hasScale) Vec3(scaleX.toDouble(), scaleY.toDouble(), scaleZ.toDouble()) else VEC3_ONES
    }
    ...
    fun spawnInWorld(level: Level, player: Player?) {
        Optional.ofNullable(EffectRegistry.get(effek)).ifPresent { effek: EffectDefinition ->
            val emitter = if (hasEmitter) effek.play(emitterName = this.emitter!!) else effek.play()
            val x: Float
            val y: Float
            val z: Float
            if (hasPosition) {
    ...
    companion object {
        private val VEC3_ONES = Vec3(1.0, 1.0, 1.0)
    }
}
