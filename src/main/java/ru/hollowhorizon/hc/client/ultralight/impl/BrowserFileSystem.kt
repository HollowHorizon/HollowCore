 ...
 */
package ru.hollowhorizon.hc.client.ultralight.impl
...
class BrowserFileSystem : UltralightFileSystem {
    ...
    private fun log(error: Boolean, fmt: String, vararg args: Any) {
        ...
        if (error) {
            HollowCore.LOGGER.error("[ERROR/FileSystem] $message")
        } else {
            HollowCore.LOGGER.debug("[INFO/FileSystem] $message")
        }
    }
}
