...
class ViewLoadListener(private val viewOverlay: ViewOverlay) : UltralightLoadListener {
    ...
    override fun onBeginLoading(frameId: Long, isMainFrame: Boolean, url: String) {
        HollowCore.LOGGER.debug("${frameName(frameId, isMainFrame, url)}The view is about to load")
    }
    ...
    override fun onFinishLoading(frameId: Long, isMainFrame: Boolean, url: String) {
        HollowCore.LOGGER.info("${frameName(frameId, isMainFrame, url)}The view finished loading")
    }
    ...
    override fun onFailLoading(
        ...
        HollowCore.LOGGER.error("${frameName(frameId, isMainFrame, url)}Failed to load $errorDomain, $errorCode($description)")
    }
