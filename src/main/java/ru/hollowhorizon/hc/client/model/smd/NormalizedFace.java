...
public class NormalizedFace {
    ...
    public void addFaceForRender(MatrixStack matrixStack, BufferBuilder bufferBuilder, int packedLight, int packedOverlay,
                                 boolean smoothShading, float partialTick, float r, float g, float b, float a) {
        ...
        for (int i = 0; i < 3; ++i) {
            IVertexBuilder vertexBuilder = bufferBuilder.vertex(poseMatrix, this.vertices[i].getX(partialTick), this.vertices[i].getY(partialTick), this.vertices[i].getZ(partialTick))
                    .color(r, g, b, a).uv(this.textureCoordinates[i].u, this.textureCoordinates[i].v).overlayCoords(packedOverlay).uv2(packedLight);
            if (!smoothShading) {
