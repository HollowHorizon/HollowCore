...
public class RenderedGltfModel {
	...
	public static final Map<NodeModel, float[]> nodeGlobalTransformLookup = new IdentityHashMap<>();
	...
	protected void processNodeModel(NodeModel nodeModel, List<Runnable> skinningCommands, List<Runnable> vanillaRenderCommands, List<Runnable> shaderModRenderCommands) {
		ArrayList<Runnable> nodeSkinningCommands = new ArrayList<>();
		ArrayList<Runnable> vanillaNodeRenderCommands = new ArrayList<>();
		ArrayList<Runnable> shaderModNodeRenderCommands = new ArrayList<>();
	...
	protected void processMeshPrimitiveModel(NodeModel nodeModel, MeshModel meshModel, MeshPrimitiveModel meshPrimitiveModel, List<Runnable> skinningCommand, List<Runnable> vanillaRenderCommands, List<Runnable> shaderModRenderCommands) {
		...
		AccessorModel positionsAccessorModel = attributes.get("POSITION");
		if(positionsAccessorModel != null) {
			List<Runnable> renderCommand = new ArrayList<>();
			AccessorModel normalsAccessorModel = attributes.get("NORMAL");
			if(normalsAccessorModel != null) {
				AccessorModel tangentsAccessorModel = attributes.get("TANGENT");
				if(tangentsAccessorModel != null) {
	...
	public static class Material {
		...
		public void initMaterialCommand(RenderedGltfModel renderedModel) {
			...
			int colorMap = baseColorTexture == null ? GlTFModelManager.getInstance().getDefaultColorMap() : renderedModel.obtainGlTexture(textureModels.get(baseColorTexture.index));
			int normalMap = normalTexture == null ? GlTFModelManager.getInstance().getDefaultNormalMap() : renderedModel.obtainGlTexture(textureModels.get(normalTexture.index));
			int specularMap = specularTexture == null ? GlTFModelManager.getInstance().getDefaultSpecularMap() : renderedModel.obtainGlTexture(textureModels.get(specularTexture.index));
			if(doubleSided) {
	...
	public int obtainGlTexture(TextureModel textureModel) {
		...
		if(glTexture == null) {
			...
			if (pixelData == null)
			{
				GlTFModelManager.logger.warn("Could not extract pixel data from image");
