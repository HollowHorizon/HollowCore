...
public class CefRenderer {
    ...
    protected void onPaint(boolean popup, Rectangle[] dirtyRects, ByteBuffer buffer, int width, int height, boolean completeReRender) {
        ...
        if (size > buffer.limit()) {
            HollowCore.LOGGER.warn("Bad data passed to CefRenderer.onPaint() triggered safe guards... (1)");
            return;
        }
        ...
        if (!popup) {
            if (completeReRender || width != view_width_ || height != view_height_) {
                ...
                view_width_ = width;
                view_height_ = height;
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, view_width_, view_height_, 0, 0x80E1, GL_UNSIGNED_BYTE, buffer);
            } else {
                ...
                for (Rectangle rect : dirtyRects) {
                    if (rect.x < 0 || rect.y < 0 || rect.x + rect.width > view_width_ || rect.y + rect.height > view_height_)
                        HollowCore.LOGGER.warn("Bad data passed to CefRenderer.onPaint() triggered safe guards... (2)");
                    else {
                        glPixelStorei(GL_UNPACK_SKIP_PIXELS, rect.x);
                        glPixelStorei(GL_UNPACK_SKIP_ROWS, rect.y);
                        glTexSubImage2D(GL_TEXTURE_2D, 0, rect.x, rect.y, rect.width, rect.height, 0x80E1, GL_UNSIGNED_BYTE, buffer);
                    }
                }
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, w, h, 0x80E1, GL_UNSIGNED_BYTE, buffer);
            glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
            glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0);
            glPixelStorei(GL_UNPACK_SKIP_ROWS, 0);
        }
