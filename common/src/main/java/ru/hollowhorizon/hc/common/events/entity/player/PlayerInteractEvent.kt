...
abstract class PlayerInteractEvent(player: Player) : PlayerEvent(player), Cancelable {
    ...
    class BlockInteract(player: Player, val hand: InteractionHand, val state: BlockHitResult) :
        PlayerInteractEvent(player)

    class ItemInteract(player: Player, val hand: InteractionHand, val itemStack: ItemStack) :
        PlayerInteractEvent(player)
}